...
const Cart = () => {
    ...
    const fetchCartData = async () => {
        try {
            ...
            // map dữ liệu từ Backend CartItem
            const mappedItems = cartItemsRaw.map((ci) => {
                const p = ci.product || ci.Product;
                return {
                    cartItemId: ci.cartItemId || ci.CartItemId,
                    productId: p?.productId || p?.ProductId,
                    productName: p?.name || p?.Name,
                    image: p?.imageUrl || p?.ImageUrl,
                    quantity: ci.quantity || ci.Quantity,
                    maxStock: p?.stock ?? p?.Stock ?? 0,
                    isChecked: true,
                    unitPrice: p?.price ?? p?.Price ?? 0,
                };
            });
    ...
    useEffect(() => {
        fetchCartData();
        window.addEventListener("auth-change", fetchCartData);
        return () => window.removeEventListener("auth-change", fetchCartData);
    }, []);
    ...
    const handleQuantityChange = async (productId, newQuantity) => {
        ...
        try {
            if (current.cartItemId) {
                await UPDATE_CART_QUANTITY(current.cartItemId, newQuantity);
            }
    ...
    const handleDeleteItem = async (productId) => {
        ...
        try {
            if (current?.cartItemId) {
                await DELETE_FROM_CART(current.cartItemId);
            }
            await fetchCartData();
        } catch (error) {
