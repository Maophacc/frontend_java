...
import { GET_ID, GET_IMG, transformProduct } from "../config/apiService"; // hoặc GET_PRODUCT_DETAIL bạn tự đặt
...
const Detail = () => {
    ...
    const fetchProduct = async () => {
        try {
            ...
            const data = await GET_ID("Product", productId);

            // Nếu apiService bạn trả axios response thì dùng data.data
            const p = transformProduct(data?.productId ? data : (data?.data || data));
            const img = p.image || p.imageUrl;
            const finalImg = (img && (img.startsWith("http") || img.startsWith("data:")))
                ? img
                : (img ? GET_IMG(img) : null);
            setProduct({ ...p, finalImg });
            setQty(1);
        } catch (e) {
